...
function StaffDetails({ staffData, setStaffData }) {
  ...
  useEffect(() => {
    const fetchStaffData = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/admin/staff/staff/add/');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        ...
        await axios.put(`http://127.0.0.1:8000/admin/staff/staff/add/${form.id}/`, form);
        setStaffData(staffData.map(staff => staff.id === form.id ? form : staff));
      } else {
        // Add new staff
        const response = await axios.post('http://127.0.0.1:8000/admin/staff/staff/add/', form);
